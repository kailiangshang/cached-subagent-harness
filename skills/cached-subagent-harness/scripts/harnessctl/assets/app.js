(() => {
  ...
  const copy = {
    "zh-CN": {
      ...
      strategyIntake: "委派门槛",
      strategyIntakeNote: "简单任务或委派净收益不为正 → 主线程；仅在收益为正时继续",
      strategyShape: "兼容分组",
      strategyShapeNote: "严格匹配并保持顺序 · 每批最多 2 项",
      strategyRoute: "能力路由",
      strategyRouteNote: "先定质量与风险下限 · 再选最低合格模型",
      strategySession: "子 Agent 会话",
      strategySessionNote: "仅在委派门槛通过后：可证明且预算充足则续接 1 次；无可复用会话才新建",
    ...
    "en-US": {
      ...
      strategyIntake: "Delegation gate",
      strategyIntakeNote: "Simple Task or no net delegation value → main; continue only when valuable",
      ...
      strategySessionNote: "Only after delegation passes: reuse once with exact proof and budget; spawn only when no eligible Session",
